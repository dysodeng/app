...
func init() {
	...
	// 初始化主Redis客户端
	mainClient, err = createRedisClient(config.Redis.Main)
	if err != nil {
		log.Fatalf("failed to create main redis client: %+v", err)
	}

	// 初始化缓存Redis客户端
	cacheClient, err = createRedisClient(config.Redis.Cache)
	if err != nil {
...
func CacheKey(key string) string {
	prefix := config.Redis.Cache.KeyPrefix
	if prefix != "" {
		key = config.Redis.Cache.KeyPrefix + ":" + key
	}
	return key
}
